...
def lr_predict(data, category_feature, continuous_feature):  # 0.47181
    ...
    print("tr正确率为:", sum(lr.predict(x_train) == y_train) / len(y_train))
    val_logloss = log_loss(y_val, lr.predict_proba(x_val)[:, 1])
    print('val-logloss: ', val_logloss)
    print("val正确率为:", sum(lr.predict(x_val) == y_val) / len(y_val))
    print('开始预测...')
...
def gbdt_predict(data, category_feature, continuous_feature):  # 0.44548
    ...
    print("tr正确率为:", sum(gbm.predict(x_train) == y_train) / len(y_train))
    print("val正确率为:", sum(gbm.predict(x_val) == y_val) / len(y_val))
